...
async function limitFind(words){
  let count = await words.count({
    word: {$regex: '^p'}
  });
  console.log("Count of words starting with p : " + count);

  let cursor = await words.find({
    // word: {$regex: '^p'}
  });
  await displayWords("Words starting with p : ", cursor);

  // cursor = await words.find({
  //   // word: {$regex: '^p'},
  // },
  // {limit:5}
  // );
  ...
  await displayWords("Limiting words starting with p : ", cursor);
}
...
async function main() {
  ...
  results = await collection.updateMany(
    {type:'words',word: 'pruslan'},
    {$set:{type:'words',word: 'pruslan'}},
    {upsert:true}
  );
